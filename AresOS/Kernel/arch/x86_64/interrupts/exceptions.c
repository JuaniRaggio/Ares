...
void exceptionDispatcher(int exception, uint64_t *stack_frame) {
        ...
        // Guardar en saved_regs para syscall
        saved_regs = regs;
...
static void print_registers(regs_snapshot_t *regs) {
        ...
        ncPrint("RAX: 0x", WHITE);
        ncPrintHex(regs->rax);
        ncPrint("  RBX: 0x", WHITE);
        ncPrintHex(regs->rbx);
        ncPrint("  RCX: 0x", WHITE);
        ncPrintHex(regs->rcx);
        ncNewline();

        ncPrint("RDX: 0x", WHITE);
        ncPrintHex(regs->rdx);
        ncPrint("  RSI: 0x", WHITE);
        ncPrintHex(regs->rsi);
        ncPrint("  RDI: 0x", WHITE);
        ncPrintHex(regs->rdi);
        ncNewline();

        ncPrint("RBP: 0x", WHITE);
        ncPrintHex(regs->rbp);
        ncPrint("  RSP: 0x", WHITE);
        ncPrintHex(regs->rsp);
        ncPrint("  RIP: 0x", WHITE);
        ncPrintHex(regs->rip);
        ncNewline();
        ...
        ncPrint("  R9:  0x", WHITE);
        ncPrintHex(regs->r9);
        ncPrint("  R10: 0x", WHITE);
        ncPrintHex(regs->r10);
        ncNewline();

        ncPrint("R11: 0x", WHITE);
        ncPrintHex(regs->r11);
        ncPrint("  R12: 0x", WHITE);
        ncPrintHex(regs->r12);
        ncPrint("  R13: 0x", WHITE);
        ncPrintHex(regs->r13);
        ncNewline();

        ncPrint("R14: 0x", WHITE);
        ncPrintHex(regs->r14);
        ncPrint("  R15: 0x", WHITE);
        ncPrintHex(regs->r15);
        ncNewline();
