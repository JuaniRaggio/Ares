...
void tron_render(void) {
    ...
    for (int y = 0; y < GRID_HEIGHT; y++) {
        for (int x = 0; x < GRID_WIDTH; x++) {
            if (game.grid[y][x] != 0) {
                uint32_t color = (game.grid[y][x] == 1) ? TRAIL_COLOR_P1 : TRAIL_COLOR_P2;
    ...
    if (game.player1.alive) {
        syscall_draw_rect(game.player1.x * CELL_SIZE, game.player1.y * CELL_SIZE,
                        CELL_SIZE, CELL_SIZE, game.player1.color);
    }

    if (game.player2.alive) {
        syscall_draw_rect(game.player2.x * CELL_SIZE, game.player2.y * CELL_SIZE,
                        CELL_SIZE, CELL_SIZE, game.player2.color);
    }
}
