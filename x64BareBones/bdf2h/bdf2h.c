...
int main(int argc, char *argv[]) {
        ...
        if (p_args.flag_h) {
                printf("Usage: bdf2h -i <in_bdf_font> -o <out_filename> "
                       "[option(s)]\n");
                printf("Converts a font in bdf format to bitmap C array "
                       "format.\n");
                printf("-h --help\tThis help text.\n");
                printf("-i <filename>\tInput font file in bdf format.\n");
                printf("-o <filename>\tOutput file to generate (default = "
                       "stdout).\n");
                printf("--hex\t\tElements in C array output are in hex "
                       "(default = binary).\n");
                printf("-n <fontName>\tFont name used to write array (default "
        ...
        if (p_args.in_fname != NULL) {
                ...
                fprintf(stderr,
                        "Use -i <filename> to specify font to convert.\n");
                return 0;
        }
                ...
                if (strcmp(argv[i], "-h") == 0 ||
                    strcmp(argv[i], "--help") == 0) {
                        ...
                        if (i + 1 < argc) {
                                ...
                                fprintf(
                                    stderr,
                                    "Error: -o flag requires a filename.\n");
                                exit(-1);
                        }
                        ...
                        if (i + 1 < argc) {
                                ...
                                fprintf(
                                    stderr,
                                    "Error: -i flag requires a filename.\n");
                                exit(-1);
                        }
                        ...
                        if (i + 1 < argc) {
                                ...
                                fprintf(
                                    stderr,
                                    "Error: -f flag requires a fontName.\n");
                                exit(-1);
                        }
                } else {
...
void write_char_line_data(char c, t_settings settings) {
        ...
        if (settings.flag_hex) {
                ...
                for (i = 0; i < 8; i++) {
                        (c & 0x01 << i) ? fputc('1', settings.out)
                                        : fputc('0', settings.out);
                }
        }
}
...
void write_char(t_settings settings, unsigned char *bitmap, int fontwidth,
        ...
        for (y = 0; y < fontheight; y++) {
                ...
                for (x = 0; x < fontwidth; x += 8) {
                        ...
                        // If current row is above or below the bitmap, output a
                        // blank row.
...
int write_check_errors(t_bdf_data bdf_data) {
        ...
        if (bdf_data.bBox_width <= 0 || bdf_data.bBox_height <= 0) {
                fprintf(stderr,
                        "Boundingbox size error. { w, h } = { %d, %d }\n",
...
void get_write_char(t_bdf_data bdf_data, t_settings settings,
        ...
        while (1) {
                ...
                if (!strcasecmp(s, "STARTCHAR")) {
                        ...
                        // fprintf(out, "// dwidth %d, bbx %d, bby %d, bbw %d,
                        // bbh %d\n", dwidth, bbx, bby, bbw, bbh);

                        if (n == bdf_data.nChars) {
                                fprintf(stderr,
                                        "Error: bdf metadata declares less "
                                        "chars han actually inside.\n");
                                exit(-1);
                        }
                        if (dwidth == INT_MIN) {
                        ...
                        // width_table[n] = width;// No width table, will use
                        // monospace. encoding_table[n] = encoding;// Will not
                        // use an encoding table.
                        n++;

                        scanline = 0;

                        memset(bitmap, 0,
                               ((bdf_data.bBox_width + 7) / 8) *
                                   bdf_data.bBox_height);
                } else if (!strcasecmp(s, "ENDCHAR")) {

                        fprintf(settings.out, "\t{\n");
                        write_char(settings, bitmap, bdf_data.bBox_width,
                                   bdf_data.bBox_height, bdf_data.bBox_yos, bbh,
                                   bby);
                        fprintf(settings.out, "\t}");
